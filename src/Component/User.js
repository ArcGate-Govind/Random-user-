...
const User = () => {
  ...
  const Adduser = () => {
    ...
    axios.get("https://randomuser.me/api/").then((response) => {
      ...
      let name =response.data.results[0].name.first +" " +response.data.results[0].name.last;

      setFullname((current) => [
        ...current,{'name': name,'wealth': ramdon}
      ]);
    ...
    <div className="container mt-5">
      <div className="d-flex align-items-start ">
        <div className="d-grid gap-2 col-4  p-2 border-end  border-2  border-dark">
          ...
          <button onClick={doubleData} className="btn btn-light border border-dark mt-2" type="button">
            Double Money &#128176;
          </button>
          <button onClick={showMillionaires} className="btn btn-light border border-dark mt-2" type="button">
            Show Only Millionaires &#128181;
          </button>
          <button onClick={sortData} className="btn btn-light border border-dark mt-2" type="button">
            Sort by Richest &#11015;
          </button>
          <button onClick={totalAmount} className="btn btn-light border border-dark mt-2" type="button">
